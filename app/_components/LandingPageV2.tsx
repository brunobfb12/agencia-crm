...
.lp h1,.lp h2,.lp h3,.lp h4{font-family:'Syne',sans-serif;line-height:1.1}
.lp h1{font-size:clamp(2.6rem,5.5vw,4.8rem);font-weight:800}
.lp h2{font-size:clamp(1.9rem,3.5vw,2.9rem);font-weight:700}
.lp h3{font-size:1.15rem;font-weight:700}
.lp .grad{background:linear-gradient(135deg,var(--indigo-l) 0%,var(--cyan) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.lp .lbl{display:inline-flex;align-items:center;gap:8px;font-size:.72rem;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--indigo-l);padding:5px 14px;border:1px solid rgba(99,102,241,.28);border-radius:100px;background:rgba(99,102,241,.07);margin-bottom:18px}
.lp .gc{background:var(--gb);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--gbd);border-radius:var(--rl);transition:var(--tr)}
.lp .gc:hover{background:var(--gbh);border-color:var(--gbdh);transform:translateY(-4px);box-shadow:0 0 50px rgba(99,102,241,.2)}
.lp .btn{display:inline-flex;align-items:center;gap:8px;padding:14px 28px;border-radius:12px;font-family:'DM Sans',sans-serif;font-weight:600;font-size:1rem;cursor:pointer;border:none;transition:var(--tr);text-decoration:none}
.lp .bp{background:linear-gradient(135deg,var(--indigo),var(--violet));color:#fff}
.lp .bp:hover{transform:translateY(-2px);box-shadow:0 8px 44px rgba(99,102,241,.5),0 0 0 1px rgba(99,102,241,.3)}
.lp .bs{background:rgba(255,255,255,.04);color:var(--txt);border:1px solid var(--gbd);backdrop-filter:blur(10px)}
.lp .bs:hover{background:rgba(255,255,255,.09);border-color:rgba(255,255,255,.18)}
.lp .ni{display:flex;align-items:center;justify-content:center;width:52px;height:52px;border-radius:14px;color:#fff;background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.22);animation:np 4s ease-in-out infinite;flex-shrink:0}
.lp .ni.c{background:rgba(34,211,238,.1);border-color:rgba(34,211,238,.22);color:rgba(34,211,238,.95);animation:npc 4s ease-in-out infinite}
.lp .ni.e{background:rgba(16,185,129,.1);border-color:rgba(16,185,129,.22);color:rgba(16,185,129,.95);animation:npe 4s ease-in-out infinite}
.lp .ni.a{background:rgba(245,158,11,.1);border-color:rgba(245,158,11,.22);color:rgba(245,158,11,.95);animation:npa 4s ease-in-out infinite}
...
.lp .si{display:flex;align-items:center;justify-content:center;width:44px;height:44px;color:rgba(99,102,241,.9);margin-bottom:12px;filter:drop-shadow(0 0 8px rgba(99,102,241,.5))}
...
.lp .fu:nth-child(2){transition-delay:.1s}.lp .fu:nth-child(3){transition-delay:.2s}.lp .fu:nth-child(4){transition-delay:.3s}.lp .fu:nth-child(5){transition-delay:.4s}.lp .fu:nth-child(6){transition-delay:.5s}
.lp nav{position:fixed;top:0;left:0;right:0;z-index:1000;padding:14px 0;backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);background:rgba(4,4,12,.82);border-bottom:1px solid var(--gbd);transition:var(--tr)}
.lp .nav-i{display:flex;align-items:center;justify-content:space-between}
.lp .logo{font-family:'Syne',sans-serif;font-size:1.35rem;font-weight:800;background:linear-gradient(135deg,var(--indigo-l),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;text-decoration:none}
.lp .nav-links{display:flex;gap:28px;list-style:none}
.lp .nav-links a{color:var(--txt2);text-decoration:none;font-size:.88rem;font-weight:500;transition:color .2s}
...
.lp .hbadge{display:inline-flex;align-items:center;gap:8px;padding:6px 14px;background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.25);border-radius:100px;font-size:.78rem;color:var(--indigo-l);margin-bottom:22px}
.lp .hbadge::before{content:'';display:block;width:7px;height:7px;background:var(--cyan);border-radius:50%;animation:blink 2.2s ease-in-out infinite}
@keyframes blink{0%,100%{opacity:1}50%{opacity:.25}}
.lp .hero-h1{margin-bottom:22px;letter-spacing:-.02em}
.lp .hero-sub{font-size:1.1rem;color:var(--txt2);line-height:1.75;margin-bottom:36px;max-width:480px}
.lp .hero-ctas{display:flex;gap:14px;flex-wrap:wrap;margin-bottom:18px}
.lp .hero-trust{font-size:.78rem;color:var(--txt3)}
...
.lp .sv{font-family:'Syne',sans-serif;font-size:1.55rem;font-weight:800;color:var(--indigo-l)}
.lp .sl{font-size:.72rem;color:var(--txt3);margin-top:4px}
.lp .phone{background:var(--gb);backdrop-filter:blur(20px);border:1px solid var(--gbd);border-radius:28px;overflow:hidden;box-shadow:0 40px 120px rgba(99,102,241,.2),0 0 0 1px rgba(99,102,241,.1);max-width:360px;margin:0 auto;position:relative}
.lp .ph-hd{padding:16px 20px;border-bottom:1px solid var(--gbd);display:flex;align-items:center;gap:12px;background:rgba(99,102,241,.06)}
.lp .ph-av{width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,var(--indigo),var(--cyan));display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:700;color:#fff;flex-shrink:0}
.lp .ph-name{font-size:.9rem;font-weight:600}
.lp .ph-st{font-size:.72rem;color:var(--emerald);display:flex;align-items:center;gap:4px}
.lp .ph-st::before{content:'';display:block;width:6px;height:6px;border-radius:50%;background:var(--emerald);animation:blink 2s ease-in-out infinite}
...
.lp .cm.u{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);align-self:flex-start;border-bottom-left-radius:4px}
.lp .cm.ai{background:linear-gradient(135deg,rgba(99,102,241,.22),rgba(124,58,237,.22));border:1px solid rgba(99,102,241,.22);align-self:flex-end;border-bottom-right-radius:4px}
.lp .cm-m{font-size:.63rem;color:var(--txt3);margin-top:3px}
.lp .cm-m.r{text-align:right}
.lp .typing{display:flex;gap:4px;padding:10px 14px;background:rgba(255,255,255,.04);border:1px solid var(--gbd);border-radius:12px;border-bottom-right-radius:4px;align-self:flex-end;width:fit-content}
.lp .td{width:6px;height:6px;border-radius:50%;background:var(--indigo-l);animation:t 1.3s ease-in-out infinite}
...
.lp .pbadge{position:absolute;top:-18px;right:-18px;padding:11px 15px;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.3);backdrop-filter:blur(20px);border-radius:14px;font-size:.73rem;z-index:10}
.lp .pbadge .bt{font-weight:700;color:var(--emerald);font-size:.78rem}
.lp .pbadge .bs2{color:var(--txt3);margin-top:2px}
#lp-ticker{padding:18px 0;border-top:1px solid var(--gbd);border-bottom:1px solid var(--gbd);overflow:hidden;position:relative;z-index:2}
.lp .tk-i{display:flex;gap:36px;animation:tk 28s linear infinite;width:max-content}
.lp .tk-item{display:flex;align-items:center;gap:10px;white-space:nowrap;font-size:.88rem;color:var(--txt2)}
.lp .tk-item strong{color:var(--txt)}
.lp .tk-dot{width:4px;height:4px;background:var(--indigo);border-radius:50%;flex-shrink:0;align-self:center}
.lp .tk-icon{width:16px;height:16px;color:var(--indigo-l);flex-shrink:0}
@keyframes tk{from{transform:translateX(0)}to{transform:translateX(-50%)}}
.lp #pain{padding:100px 0}
.lp .pain-header{text-align:center;margin-bottom:56px}
.lp .pain-sub{color:var(--txt2);margin-top:14px;max-width:540px;margin-left:auto;margin-right:auto;font-size:1.05rem}
.lp .pain-grid{display:grid;grid-template-columns:1fr 1fr;gap:22px}
.lp .pcol{padding:32px;border-radius:var(--rl)}
...
.lp .pcol.good .pcol-t{color:var(--emerald)}
.lp .pl{list-style:none;display:flex;flex-direction:column;gap:11px}
.lp .pl li{display:flex;gap:10px;font-size:.88rem;color:var(--txt2);line-height:1.5}
.lp .pl li .ic{flex-shrink:0}
.lp #hiw{padding:100px 0 60px}
.lp .hiw-hd{margin-bottom:44px}
.lp .sdiv{width:55px;height:3px;background:linear-gradient(90deg,var(--indigo),var(--cyan));border-radius:2px;margin-top:14px}
.lp .sw{overflow-x:auto;padding-bottom:20px;scrollbar-width:thin;scrollbar-color:rgba(99,102,241,.3) transparent}
...
.lp .scard{width:210px;flex-shrink:0;padding:24px;border-radius:var(--rl)}
.lp .sn{font-family:'Syne',sans-serif;font-size:.7rem;font-weight:700;color:var(--indigo-l);letter-spacing:.1em;margin-bottom:10px;opacity:.65}
.lp .st2{font-size:.95rem;font-weight:700;margin-bottom:7px;font-family:'Syne',sans-serif}
.lp .sd{font-size:.8rem;color:var(--txt2);line-height:1.5}
.lp .sarr{width:18px;flex-shrink:0;display:flex;align-items:center;justify-content:center;color:var(--indigo-l);opacity:.35;align-self:center}
...
.lp .fd{font-size:.86rem;color:var(--txt2);line-height:1.65}
.lp .hbox{padding:52px 48px;text-align:center;border-radius:32px;background:linear-gradient(135deg,rgba(99,102,241,.09) 0%,rgba(124,58,237,.07) 50%,rgba(34,211,238,.05) 100%);border:1px solid rgba(99,102,241,.22);position:relative;overflow:hidden;margin:60px 0}
.lp .hbox::before{content:'';position:absolute;top:0;left:50%;transform:translateX(-50%);width:600px;height:1px;background:linear-gradient(90deg,transparent,rgba(99,102,241,.5),transparent)}
.lp .hbox-icon{margin-bottom:14px;display:flex;align-items:center;justify-content:center}
.lp .hbox h2{max-width:680px;margin:0 auto 14px}
.lp .hbox p{color:var(--txt2);font-size:1.05rem;max-width:580px;margin:0 auto 30px;line-height:1.75}
.lp #testi{padding:100px 0}
.lp .tw{overflow-x:auto;padding-bottom:20px;margin-top:46px;scrollbar-width:thin;scrollbar-color:rgba(99,102,241,.3) transparent}
...
.lp .tstars{font-size:.88rem;color:var(--amber);margin-bottom:14px;filter:drop-shadow(0 0 5px rgba(245,158,11,.4))}
.lp .ttxt{font-size:.9rem;color:var(--txt2);line-height:1.75;margin-bottom:20px;font-style:italic}
.lp .tauth{display:flex;align-items:center;gap:12px}
.lp .tav{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.88rem;color:#fff;flex-shrink:0;background:linear-gradient(135deg,var(--indigo),var(--violet))}
.lp .tn{font-weight:600;font-size:.88rem}
.lp .tr2{font-size:.73rem;color:var(--txt3)}
...
.lp .pcard.feat{background:rgba(99,102,241,.08);border-color:rgba(99,102,241,.36);box-shadow:0 0 70px rgba(99,102,241,.15)}
.lp .pcard.feat::before{content:'MAIS POPULAR';position:absolute;top:-11px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,var(--indigo),var(--violet));color:#fff;font-size:.63rem;font-weight:700;letter-spacing:.12em;padding:4px 12px;border-radius:100px;white-space:nowrap}
.lp .pname{font-family:'Syne',sans-serif;font-size:1.7rem;font-weight:800;color:var(--txt);margin-bottom:6px;letter-spacing:-.02em;padding-bottom:14px;border-bottom:1px solid var(--gbd)}
.lp .pcard.feat .pname{background:linear-gradient(135deg,var(--indigo-l),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.lp .pdesc{font-size:.82rem;color:var(--txt3);margin:10px 0 18px}
.lp .pprice{margin-bottom:4px}
.lp .pcur{font-size:.95rem;color:var(--indigo-l);vertical-align:top;margin-top:8px;display:inline-block}
.lp .pamt{font-family:'Syne',sans-serif;font-size:2.9rem;font-weight:800;color:var(--txt)}
.lp .pper{font-size:.82rem;color:var(--txt3)}
.lp .panual{font-size:.73rem;color:var(--txt3);margin-bottom:20px}
.lp .pfl{list-style:none;display:flex;flex-direction:column;gap:9px;margin-bottom:26px}
.lp .pfl li{display:flex;align-items:flex-start;gap:8px;font-size:.83rem;color:var(--txt2)}
.lp .pfl li::before{content:'✓';color:var(--emerald);font-weight:700;flex-shrink:0;filter:drop-shadow(0 0 3px rgba(16,185,129,.4));margin-top:1px}
.lp .pfl li.star::before{content:'★';color:var(--amber);filter:drop-shadow(0 0 4px rgba(245,158,11,.5))}
.lp .pfl li.note{color:var(--txt3);font-size:.77rem;padding-left:16px}
...
.lp .sc2 p{color:var(--txt2);margin-top:12px;font-size:1rem;max-width:560px;margin-left:auto;margin-right:auto}
.lp .fql{display:flex;flex-direction:column;gap:10px;margin-top:40px;max-width:720px;margin-left:auto;margin-right:auto}
.lp .fqi{padding:20px 24px;cursor:pointer;user-select:none}
.lp .fqq{display:flex;justify-content:space-between;align-items:center;gap:16px;font-size:.93rem;font-weight:600;color:var(--txt)}
.lp .fqtg{font-size:1.4rem;font-weight:300;color:var(--indigo-l);flex-shrink:0;transition:transform .3s}
.lp .fqi.open .fqtg{transform:rotate(45deg)}
.lp .fqa{font-size:.86rem;color:var(--txt2);line-height:1.7;max-height:0;overflow:hidden;transition:max-height .4s ease,margin-top .3s}
.lp .fqi.open .fqa{max-height:400px;margin-top:14px}
.lp #ctaf{padding:100px 0}
.lp .ctabox{border-radius:32px;background:linear-gradient(135deg,rgba(99,102,241,.1) 0%,rgba(124,58,237,.08) 50%,rgba(34,211,238,.05) 100%);border:1px solid rgba(99,102,241,.25);padding:64px 48px;text-align:center;position:relative;overflow:hidden}
.lp .ctabox::before{content:'';position:absolute;top:0;left:50%;transform:translateX(-50%);width:600px;height:1px;background:linear-gradient(90deg,transparent,rgba(99,102,241,.6),transparent)}
.lp .ctrust{display:flex;flex-wrap:wrap;justify-content:center;gap:14px 24px;margin-top:20px}
.lp .ctrust span{font-size:.8rem;color:var(--txt3)}
.lp footer{padding:32px 0;border-top:1px solid var(--gbd);position:relative;z-index:2}
.lp .fi{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px}
.lp .fc{font-size:.78rem;color:var(--txt3)}
.lp .fc a{color:var(--txt3);text-decoration:none}
.lp .fc a:hover{color:var(--indigo-l)}
...
const DEPOIMENTOS = [
  { av:"TP", avStyle:{},                                                               nome:"Thaísy P.",    cargo:"Proprietária — Studio de Sobrancelhas",  txt:"Em 2 semanas minha agenda estava lotada. Antes perdia cliente por não responder rápido — trabalhava em atendimentos e o WhatsApp ficava mudo. Agora a IA responde, qualifica e já manda o link de agendamento. Eu entro pra atender quem já está decidido." },
  { av:"MT", avStyle:{background:"linear-gradient(135deg,#22d3ee,#10b981)"},           nome:"Marcelo T.",   cargo:"Gestor Comercial — Loja de Tintas",       txt:"Recebia 80 mensagens por dia e o vendedor não dava conta. Com o FácilCRM a IA filtra, responde e passa pro vendedor só quem está quente. Resultado: conversão subiu 40% em 3 semanas e o vendedor parou de reclamar de sobrecarga." },
  { av:"CS", avStyle:{background:"linear-gradient(135deg,#7c3aed,#ec4899)"},           nome:"Carla S.",     cargo:"Diretora Clínica — Odontologia Estética",  txt:"O follow-up automático trouxe de volta 11 pacientes que tinham sumido há mais de 3 meses. O sistema mandou a mensagem certa na hora certa — eu nem sabia que essas pessoas ainda tinham interesse. Recuperamos R$4.800 em consultas que estavam perdidas." },
  { av:"AP", avStyle:{background:"linear-gradient(135deg,#f59e0b,#f97316)"},           nome:"Ana Paula R.", cargo:"Sócia — Boutique de Moda Feminina",        txt:"A IA parabenizou uma cliente antiga que a gente não falava há 4 meses. Ela respondeu pedindo orçamento de R$1.200. Só isso já pagou mais de 3 meses de plano. Agora imagina isso acontecendo com toda a base, todo mês, no piloto automático." },
];
...
export default function LandingPageV2() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      if (nav) nav.style.background = window.scrollY > 50 ? "rgba(4,4,12,.96)" : "rgba(4,4,12,.82)";
    };
        ...
        <div className="container">
          <div className="nav-i">
            <div>
              ...
              <div style={{fontSize:".65rem",color:"var(--txt3)",letterSpacing:".04em",marginTop:"1px"}}>A IA que atende. O vendedor que fecha.</div>
            </div>
        ...
        <div className="container">
          <div className="hero-grid">
            <div className="hero-content">
              ...
              <p style={{fontSize:".9rem",color:"var(--indigo-l)",fontWeight:600,letterSpacing:".03em",marginBottom:"18px",marginTop:"-10px"}}>
        ...
        <div className="tk-i">
          {[0, 1].flatMap((rep) =>
            TICKER_ITEMS.flatMap((item, i) => [
              <div key={`item-${rep}-${i}`} className="tk-item">
                <svg className="tk-icon"><use href={`#${item.icon}`}/></svg>
                <span>{item.label}</span>
              </div>,
              <div key={`dot-${rep}-${i}`} className="tk-dot" />,
            ])
          )}
        </div>
      </div>
      ...
      <section id="hiw">
        <div className="container">
          <div className="hiw-hd fu">
            ...
            <p style={{color:"var(--txt2)",marginTop:"10px",fontSize:".98rem"}}>Enquanto você dorme, a IA está aquecendo seus leads. Quando você acorda, o vendedor já tem quem fechar.</p>
            <div className="sdiv" />
          </div>
        </div>
      ...
      <section id="features">
        <div className="container">
          <div className="sc2 fu">
            ...
            <p>Cada funcionalidade foi construída para resolver um problema real de quem vende pelo WhatsApp todo dia.</p>
          </div>
          <div className="fg">
            {FEATURES.map((f) => (
            ...
            <div className="tt">
              {DEPOIMENTOS.map((d) => (
                <div key={d.nome} className="gc tcard">
                  ...
                  <div className="tauth">
                    <div className="tav" style={d.avStyle}>{d.av}</div>
      ...
      <section id="pricing">
        <div className="container">
          <div className="sc2 fu">
            ...
            <p>Se você perde R$500 por mês em leads sem resposta, o FácilCRM se paga na primeira semana. 30 dias grátis — sem cartão.</p>
          </div>
          <div className="pg">
          ...
          <p style={{textAlign:"center",color:"var(--txt3)",fontSize:".83rem",marginTop:"22px"}}>
            Plano anual = <strong style={{color:"var(--indigo-l)"}}>2 meses grátis</strong> · Cancele quando quiser · Sem fidelidade
          </p>
        </div>
      </section>
      ...
      <section id="ctaf">
        <div className="container">
          <div className="ctabox fu">
            ...
            <p style={{color:"var(--txt2)",fontSize:"1.05rem",maxWidth:"580px",margin:"16px auto 30px",lineHeight:1.75}}>
            ...
            <p style={{color:"var(--txt3)",fontSize:".78rem",marginTop:"24px",maxWidth:"480px",marginLeft:"auto",marginRight:"auto"}}>
